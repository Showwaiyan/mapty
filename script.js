...
class Cycling extends Workout {
    ...
    constructor(coords, distance, duration, elevation, location) {
        super(coords, distance, duration);
...
class App {
    ...
    async _getLocation(lat, lng) {
        try {
            const res = await fetch(
                `https://api-bdc.net/data/reverse-geocode?latitude=${lat}&longitude=${lng}&localityLanguage=en&key=bdc_411bb5e0273f4d2fa208c27bb714d739`
            );
    ...
    async _newWorkout(e) {
        ...
        if (type === "running") {
            ...
            workout = new Running([lat, lng], distance, duration, cadence, location);
        ...
        let html = `
            <li class="workout workout--${workout.type}" data-id="${workout.id}">
                <div class="workout__nav">
                    <h2 class="workout__title">${workout.description}<span>(${workout.location})</span></h2>
                    <div>
    ...
    _editWorkout(e) {
        ...
        this.#currentEditWorkout = this.#workouts.find(
            (workout) => workout.id === this.#currentEditWorkoutEl.dataset.id
        );
    ...
    _updateWorkout() {
        ...
        if (!this._isNumber(+inputDistance.value, +inputDuration.value))
            return alert("Please enter the valid number input and positive number!");
        ...
        this.#currentEditWorkoutEl.querySelector(".workout__icon").innerText =
            this.#currentEditWorkout.type === "running" ? "🏃‍♂" : "🚴‍♀️";
        this.#currentEditWorkoutEl.querySelector(".workout__value--distance").innerText =
            this.#currentEditWorkout.distance;
        this.#currentEditWorkoutEl.querySelector(".workout__value--duration").innerText =
            this.#currentEditWorkout.duration;

        if (this.#currentEditWorkout.type === "running") {
            this.#currentEditWorkoutEl.querySelector(".workout__value--pace").innerText =
                this.#currentEditWorkout.pace.toFixed(1);
            this.#currentEditWorkoutEl.querySelector(".workout__value--cadence").innerText =
                this.#currentEditWorkout.cadence;
        } else {
            this.#currentEditWorkoutEl.querySelector(".workout__value--speed").innerText =
                this.#currentEditWorkout.speed.toFixed(1);
            this.#currentEditWorkoutEl.querySelector(".workout__value--elevation").innerText =
                this.#currentEditWorkout.elevation;
        }
